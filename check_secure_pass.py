...
def calculate_time_cracking(password):
    ...
    combination = len(string.printable[:-6]) ** len(password) 

    return combination / attemps_per_second


def generate_password(length):
    return ''.join(random.choice(string.printable[:-6]) for _ in range(length))
...
if __name__ == '__main__':
    ...
    print()
    strong_password = generate_password(int(input("what is you password length: ")))
